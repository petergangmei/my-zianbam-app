...
//                editor.putString("storyid", storyids.get(counter));
//                editor.putString("title", "views");
//                editor.apply();
//                ((FragmentActivity)getApplicationContext()).getSupportFragmentManager().beginTransaction().replace(R.id.frame,
//                        new ViewsFragment()).commit();
            }
        });
    ...
    @Override
    public void onNext() {
        Picasso.get().load(images.get(++counter)).placeholder(R.drawable.image_placeholder).into(image);
